            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4 sm:mb-6">
                        <h3 class="text-base sm:text-lg font-medium text-gray-800">Daftar Wali Kelas</h3>
                        <a href="{{ route('pengurus.kelola-walikelas.create') }}"
                            class="inline-flex items-center px-3 py-2 sm:px-4 sm:py-2 bg-green-600 rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 transition">
                            + Tambah Wali Kelas
                        </a>
                    </div>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if ($kelas->waliKelas)
                                            <div class="flex items-center gap-2 mt-1">
                                                ...
                                                <span class="text-sm text-gray-700">{{ $kelas->waliKelas->guru->nama }}</span>
                                            </div>
                                            <span class="mt-1 inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">Sudah Ditugaskan</span>
                                        @else
                                            <span class="mt-1 inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-600">Belum Ditugaskan</span>
                                        @endif
                                    </div>
                                    <div class="flex-shrink-0 ml-3">
                                        @if ($kelas->waliKelas)
                                            <form action="{{ route('pengurus.kelola-walikelas.destroy', $kelas->waliKelas->id) }}"
                                                method="POST" onsubmit="return confirm('Hapus wali kelas {{ $kelas->nama_kelas }}?');">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded text-xs">Hapus</button>
                                            </form>
                                        @else
                                            <a href="{{ route('pengurus.kelola-walikelas.create') }}"
                                                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded text-xs">Tugaskan</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        @empty
                            <p class="text-center text-gray-400 italic py-4">Tidak ada data kelas.</p>
                        @endforelse
                    </div>
                    ...
                    <div class="hidden md:block overflow-x-auto">
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                ...
                                @empty
                                    <tr><td colspan="5" class="text-center py-8 text-gray-400 italic">Tidak ada data kelas.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
