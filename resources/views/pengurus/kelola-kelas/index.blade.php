            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    <div class="flex justify-end mb-4">
                        <a href="{{ route('pengurus.kelola-kelas.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">
                            + Tambah Kelas
                        </a>
                    </div>
                    ...
                    <div class="block md:hidden space-y-3">
                        @forelse ($dataKelas as $kelas)
                            <div class="border rounded-lg p-4 bg-gray-50 flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('pengurus.kelola-kelas.destroy', $kelas) }}" method="POST" onsubmit="return confirm('Hapus data ini?');">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            <p class="text-center text-gray-500 py-4">Data kelas tidak ditemukan.</p>
                        @endforelse
                    </div>
                    ...
                    <div class="hidden md:block overflow-x-auto">
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                ...
                                @empty
                                    <tr><td colspan="3" class="text-center py-4 text-gray-500">Data kelas tidak ditemukan.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>

                    <div class="mt-4">{{ $dataKelas->links() }}</div>
                </div>
            </div>
        </div>
    </div>
