        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900" 
                    ...
                    @endif
                    <form method="POST" action="{{ route('pengurus.kelola-jadwal.store') }}">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="materi_id" id="materi_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                                    ...
                                    @foreach ($materi as $m)
                                        <option value="{{ $m->id }}" {{ old('materi_id') == $m->id ? 'selected' : '' }}>{{ $m->nama_materi }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="guru_id" id="guru_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                                    ...
                                    <template x-for="guru in guruList" :key="guru.id">
                                        <option :value="guru.id" :selected="guru.id == '{{ old('guru_id') }}'" x-text="guru.nama"></option>
                                    </template>
                                </select>
                            </div>
                            <div>
                                <label for="jam_mulai" class="block font-medium text-sm text-gray-700">Jam Mulai</label>
                                <input id="jam_mulai" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" type="time" name="jam_mulai" value="{{ old('jam_mulai') }}" required />
                            </div>
                            <div>
                                <label for="jam_selesai" class="block font-medium text-sm text-gray-700">Jam Selesai</label>
                                <input id="jam_selesai" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" type="time" name="jam_selesai" value="{{ old('jam_selesai') }}" required />
                            </div>
                        </div>
