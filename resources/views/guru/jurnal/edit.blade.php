    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-amber-500 text-white rounded-xl p-5 mb-6 shadow-lg shadow-amber-200 flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-semibold uppercase opacity-75 mb-0.5">Edit Jurnal Mengajar</p>
            ...
            <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-100">
                    <h3 class="font-bold text-gray-700">Edit Jurnal Mengajar</h3>
                    <p class="text-xs text-gray-400 mt-0.5">Perbarui catatan jurnal mengajar Anda pada tanggal tersebut.</p>
                </div>

                <form action="{{ route('guru.jurnal.update', $jadwal) }}" method="POST" class="p-6 space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="topik" id="topik"
                               value="{{ old('topik', $jurnal->topik) }}"
                               placeholder="Contoh: Surat Al-Fatihah, Huruf Hijaiyah Alif-Ba, dsb."
                               class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-sm transition
                    ...
                    <div>
                        ...
                        <select name="metode" id="metode"
                            ...
                            @foreach ($metodeOptions as $metode)
                                <option value="{{ $metode }}" {{ old('metode', $jurnal->metode) == $metode ? 'selected' : '' }}>
                                    {{ $metode }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="ringkasan" id="ringkasan" rows="5"
                                  ...
                                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-sm transition
                                         @error('ringkasan') border-red-400 @enderror">{{ old('ringkasan', $jurnal->ringkasan) }}</textarea>
                        @error('ringkasan')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="catatan" id="catatan" rows="3"
                                  ...
                                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-sm transition
                                         @error('catatan') border-red-400 @enderror">{{ old('catatan', $jurnal->catatan) }}</textarea>
                        @error('catatan')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                        ...
                        <button type="submit"
                                class="inline-flex items-center gap-2 px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded-lg shadow-md shadow-amber-200 transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
